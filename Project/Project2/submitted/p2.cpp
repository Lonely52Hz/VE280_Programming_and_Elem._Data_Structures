...
int main(int argc, char* argv[]) {
	...
	for (unsigned int i = 0; i < usernum; ++i) {
		ifstream user_info;
		user_info.open(userdir + '/' + user[i].username + '/' + "user_info");

		// check whether the file is opened or not
		try {
			if (!user_info) {
				ostringstream oStream;
				oStream << "Error: Cannot open file " << userdir + '/' + user[i].username + '/' + "user_info" << '!' << endl;
				throw Exception_t(FILE_MISSING, oStream.str());
			}
		}
		catch (Exception_t& exception) {
	...
	for (unsigned int i = 0; i < usernum; ++i) {
		for (unsigned int j = 0; j < user[i].num_posts; ++j) {
			ifstream post;
			post.open(userdir + '/' + user[i].username + "/posts/" + to_string(j + 1));

			// check whether it can be opened or not
			try {
				if (!post) {
					ostringstream oStream;
					oStream << "Error: Cannot open file " << userdir + '/' + user[i].username + "/posts/" + to_string(j + 1) << '!' << endl;
					throw Exception_t(FILE_MISSING, oStream.str());
				}
			}
			catch (Exception_t& exception) {
