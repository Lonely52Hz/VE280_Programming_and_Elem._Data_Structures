...
void visit(User_t user[], string user1, string user2, unsigned int usernum) {
    ...
    else if (isFollowing == 1 && isMutualFollowing == 1) {
...
void unfollow(User_t user[], string user1, string user2, unsigned int usernum) {
    ...
    user[user1id].num_following--;
    while (followid < user[user1id].num_following) {
        user[user1id].following[followid] = user[user1id].following[followid + 1];
        followid++;
    }
    ...
    user[user2id].num_followers--;
    while(followerid < user[user2id].num_followers) {
        user[user2id].follower[followerid] = user[user2id].follower[followerid + 1];
        followerid++;
    }
}
...
void unlike(User_t user[], string user1, string user2, unsigned int postid, unsigned int usernum) {
    ...
    try {
        ...
        if (isUnlike == 1) {
            ...
            user[user2id].posts[postid - 1].num_likes--;
            while (likeid < user[user2id].posts[postid - 1].num_likes) {
                user[user2id].posts[postid - 1].like_users[likeid] = user[user2id].posts[postid - 1].like_users[likeid + 1];
                likeid++;
            }
        }
    }
...
void uncomment(User_t user[], string user1, string user2, unsigned int postid, unsigned int commentid, unsigned int usernum) {
    ...
    try {
        ...
        if (isOwner == 0) {
            ...
            user[user2id].posts[postid - 1].num_comments--;
            while (commentid - 1 < user[user2id].posts[postid - 1].num_comments) {
                user[user2id].posts[postid - 1].comments[commentid - 1] = user[user2id].posts[postid - 1].comments[commentid];
                commentid++;
            }
        }
    }
...
void unpost(User_t user[], string user1, unsigned int postid, unsigned int usernum) {
    ...
    try {
        ...
        if (user[user1id].num_posts < postid) {
            ...
            while (postid - 1 < user[user1id].num_posts) {
                user[user1id].posts[postid - 1] = user[user1id].posts[postid];
                postid++;
            }
        }
    }
