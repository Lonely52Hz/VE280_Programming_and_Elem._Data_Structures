...
bool isDateEarly(int month1, int day1, int month2, int day2)
{
    if (month1 < month2)
    {
        return true;
...
// EFFECTS: removes Task index of type
{
    ...
    for (i = 0; i < numTasks; i++)
    {
        if (tasks[i].type == type && tasks[i].index == index)
        {
            if (isDateEarly(finish_month, finish_day, tasks[i].due_month, tasks[i].due_day))
            {
...
//          do nothing if its due_month/due_day is unchanged.
{
    ...
    for (int i = 0; i < numTasks - 1; i++)
    {
        ...
        {
            if (!isDateEarly(tasks[j].due_month, tasks[j].due_day, tasks[j + 1].due_month, tasks[j + 1].due_day))
            {
                temp = tasks[j];
                tasks[j] = tasks[j + 1];
                tasks[j + 1] = temp;
