...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        else if (!read_type.compare("DeclListNode")) {
            nodes[i] = new DeclListNode(num_child);
        }
        else if (!read_type.compare("FormalsListNode")) {
            nodes[i] = new FormalsListNode(num_child);
        }
        else if (!read_type.compare("FnBodyNode")) {
            nodes[i] = new FnBodyNode(num_child);
        }
        else if (!read_type.compare("StmtListNode")) {
            nodes[i] = new StmtListNode(num_child);
        }
        else if (!read_type.compare("VarDeclNode")) {
            nodes[i] = new VarDeclNode(num_child);
        }
        else if (!read_type.compare("FnDeclNode")) {
            nodes[i] = new FnDeclNode(num_child);
        }
        else if (!read_type.compare("FormalDeclNode")) {
            nodes[i] = new FormalDeclNode(num_child);
        }
        else if (!read_type.compare("IntNode")) {
            nodes[i] = new IntNode(num_child);
        }
        else if (!read_type.compare("BoolNode")) {
            nodes[i] = new BoolNode(num_child);
        }
        else if (!read_type.compare("VoidNode")) {
            nodes[i] = new VoidNode(num_child);
        }
        else if (!read_type.compare("AssignStmtNode")) {
            nodes[i] = new AssignStmtNode(num_child);
        }
        else if (!read_type.compare("PostIncStmtNode")) {
            nodes[i] = new PostIncStmtNode(num_child);
        }
        else if (!read_type.compare("PostDecStmtNode")) {
            nodes[i] = new PostDecStmtNode(num_child);
        }
        else if (!read_type.compare("IntLitNode")) {
            int read_val = 0;
            cin >> read_val;
            nodes[i] = new IntLitNode(num_child, read_val);
        }
        else if (!read_type.compare("TrueNode")) {
            nodes[i] = new TrueNode(num_child);
        }
        else if (!read_type.compare("FalseNode")) {
            nodes[i] = new FalseNode(num_child);
        }
        else if (!read_type.compare("IdNode")) {
            string read_myName;
            cin >> read_myName;
            nodes[i] = new IdNode(num_child, read_myName);
        }
        else if (!read_type.compare("AssignNode")) {
